...
# from .RegDCCNN import get_graph_feature
...
class Model(paddle.nn.Layer):
    def __init__(
        ...
        if self.use_graph:
            ...
            self.bn1 = nn.BatchNorm2D(dim_list[0])
            self.bn2 = nn.BatchNorm2D(dim_list[1])
            self.bn3 = nn.BatchNorm2D(dim_list[2])
            self.bn4 = nn.BatchNorm2D(dim_list[3])
            self.bn5 = nn.BatchNorm1D(n_hidden)
            self.edgeconv = nn.LayerList([
            ...
            self.final_conv = nn.Sequential(
                nn.Conv1D(sum(dim_list), n_hidden, kernel_size=1, bias_attr=False),
                self.bn5,
                nn.LeakyReLU(negative_slope=0.2))
        else:
            self.preprocess = (
    ...
    def forward(self, x):
        if self.use_graph:
            x = x.transpose([0, 2, 1])
            feature_list = []
            for conv in self.edgeconv:
                x = get_graph_feature(x, k=self.k)  # (batch_size, 3, num_points) -> (batch_size, 3*2, num_points, k)
                x = conv(x)  # (batch_size, 3*2, num_points, k) -> (batch_size, 256, num_points, k)
                # Global max pooling
                x = paddle.max(x, axis=-1)
                feature_list.append(x)
            x = self.final_conv(paddle.concat(feature_list, axis=1))
            fx = x.transpose([0, 2, 1]) + self.placeholder
        else:
