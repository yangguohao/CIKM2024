...
class AbstractUNet(BaseModel):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid,
                 ...
                 pool_kernel_size=2, conv_padding=1, is3d=True, use_attn=True):
        ...
        if self.use_attn:
            ...
            self.conv_list1 = nn.LayerList([
                SingleConv(in_channels=1024, out_channels=1024, kernel_size=2,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=2),
                SingleConv(in_channels=512, out_channels=512, kernel_size=2,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=2),
                SingleConv(in_channels=256, out_channels=256, kernel_size=2,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=2),
                SingleConv(in_channels=128, out_channels=128, kernel_size=2,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=2),
                # SingleConv(in_channels=64, out_channels=64, kernel_size=8,
                #            order='cbr', num_groups=8, padding=0, is3d=True, stride=8)
            ])

            self.upconv_list = nn.LayerList([
                nn.Conv3DTranspose(1024, 1024, kernel_size=2, stride=2, ),
                nn.Conv3DTranspose(512, 512, kernel_size=2, stride=2, ),
                nn.Conv3DTranspose(256, 256, kernel_size=2, stride=2, ),
                nn.Conv3DTranspose(128, 128, kernel_size=2, stride=2, ),
                # nn.Conv3DTranspose(64, 64, kernel_size=8, stride=8, ),
            ])
            self.conv_list2 = nn.LayerList([
                SingleConv(in_channels=2048, out_channels=2048, kernel_size=1,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=1),
                SingleConv(in_channels=1024, out_channels=1024, kernel_size=1,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=1),
                SingleConv(in_channels=512, out_channels=512, kernel_size=1,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=1),
                SingleConv(in_channels=256, out_channels=256, kernel_size=1,
                           order='cbr', num_groups=8, padding=0, is3d=True, stride=1),
                # SingleConv(in_channels=128, out_channels=128, kernel_size=4,
    ...
    def forward(self, x):
        ...
        for i, (decoder, encoder_features) in enumerate(zip(self.decoders, encoders_features)):
            if self.use_attn and i < len(encoders_features) - 1:
                encoder_features += PositionalEncodingPermute3D(encoder_features.shape[1])(encoder_features)
                x += PositionalEncodingPermute3D(x.shape[1])(x)
                conv_x = self.conv_list2[i](x).reshape((x.shape[0], x.shape[1], -1))
                conv_encoder_features = self.conv_list1[i](encoder_features)
                conv_encoder_shape = conv_encoder_features.shape
                encoder_features *= self.upconv_list[i](
                    nn.functional.sigmoid(
                        self.mhca_list[i](
                            conv_encoder_features.reshape((encoder_features.shape[0], encoder_features.shape[1], -1)),
                            conv_x,
                            conv_x
...
class UNet3D(AbstractUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=False,
                 ...
                 is_segmentation=False, conv_padding=1, use_attn=True, **kwargs):
        ...
        out_channels, final_sigmoid=final_sigmoid, basic_module=
                                     ...
                                     is_segmentation, conv_padding=conv_padding, is3d=True, use_attn=use_attn)
...
class UNet3DWithSamplePoints(UNet3D):
    ...
    int, num_levels: int, use_position_input: bool = True):
        ...
        self.final_mlp = paddle.nn.Sequential(paddle.nn.Linear(in_features=
                                                               hidden_channels, out_features=hidden_channels),
                                              paddle.nn.GELU(
...
if __name__ == '__main__':
    ...
    model = UNet3D(in_channels=4, out_channels=64, num_levels=6)
